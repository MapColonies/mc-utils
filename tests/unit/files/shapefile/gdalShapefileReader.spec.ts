...
const mockGdal = gdal as jest.Mocked<typeof gdal>;

describe('GdalShapefileReader', () => {
  let mockDataset: jest.Mocked<gdal.Dataset>;
  let mockLayer: jest.Mocked<gdal.Layer>;
  let mockLayerFeatures: jest.Mocked<gdal.LayerFeatures>;

  const shapefilePath = '/path/to/shapefile.shp';

  const createMockGdalFeature = (fid: number, properties: Record<string, unknown>, geometry: object | null): jest.Mocked<gdal.Feature> => {
    const mockGeometry = geometry
      ? ({
          toObject: jest.fn().mockReturnValue(geometry),
        } as unknown as jest.Mocked<gdal.Geometry>)
      : null;

    return {
      fid,
      fields: {
        toObject: jest.fn().mockReturnValue(properties),
      },
      getGeometry: jest.fn().mockReturnValue(mockGeometry),
    } as unknown as jest.Mocked<gdal.Feature>;
  };
  ...
  beforeEach(() => {
    ...
    mockLayerFeatures = {
      ...
      next: jest.fn(),
      count: jest.fn(),
    } as unknown as jest.Mocked<gdal.LayerFeatures>;

    // Setup mock layer
    mockLayer = {
      features: mockLayerFeatures,
    } as unknown as jest.Mocked<gdal.Layer>;
    ...
    mockDataset = {
      ...
      close: jest.fn(),
    } as unknown as jest.Mocked<gdal.Dataset>;
  ...
  it('should allow reading features through the returned interface', async () => {
    ...
    const mockGdalFeature = {
      ...
      getGeometry: jest.fn().mockReturnValue({
        toObject: jest.fn().mockReturnValue(mockGeometry),
      }),
    } as unknown as jest.Mocked<gdal.Feature>;
